...
React mobile-first profile UI components for ownport.

## Installation

```sh
pnpm add @ownport/ui @ownport/core react react-dom
# or
npm install @ownport/ui @ownport/core react react-dom
```

`@ownport/ui` is an ESM-only package targeting React 19+. It treats `react` and `react-dom` as `peerDependencies`, so the consumer installs them.
...
import { OwnportProfile } from '@ownport/ui';

const result = validate(rawJson);
if (!result.ok) throw new Error('invalid ownport.json');

const data = resolveLocale(result.data, 'en');

export function App() {
  return <OwnportProfile data={data} />;
}
```
