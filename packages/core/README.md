...
```sh
pnpm add @meport/core
# or
npm install @meport/core
```

`@meport/core` is an ESM-only package and targets Node.js 22+, modern browsers, and Cloudflare Workers. It has no peer dependencies.

## Usage

### Validate

`validate()` checks an unknown value against the bundled JSON Schema and returns a discriminated result. On success the value is narrowed to `Meport`; on failure every issue is reported with a JSON Pointer and the failing Ajv keyword.

```ts
import { validate } from '@meport/core';
import type { Meport } from '@meport/core';

const result = validate(rawJson);

if (result.ok) {
  const profile: Meport = result.data;
  // …
} else {
...
```ts
import { schema, SCHEMA_VERSION, SUPPORTED_SCHEMA_VERSIONS } from '@meport/core';
```
...
```ts
import { normalize } from '@meport/core';
...
```ts
import { normalize, resolveLocale } from '@meport/core';
...
```ts
import { generateJsonLd } from '@meport/core';
...
`MeportStorage` and `MeportAssetStorage` define the persistence contracts that adapters (Cloudflare KV/R2, filesystem, …) implement. `@meport/core` ships only the types and the `StorageError` / `NotFoundError` / `ConflictError` exception family — there is no built-in adapter yet. Concrete adapters land in Phase 3.

```ts
import { ConflictError } from '@meport/core';
import type { MeportStorage } from '@meport/core';

declare const storage: MeportStorage;
