...
Schema is a non-breaking additive bump (1.3.0 → 1.4.0): a valid 1.3.0 document is a valid 1.4.0 document unchanged, and the `v1.3.0-to-v1.4.0` migration is a pure version stamp. **Behavior change for turnkey deployments:** upgrading a Cloudflare/Vercel/static/WordPress deployment to 1.4.0 changes the _rendered look_ of the profile page (bio-style default layout and section order, built-in dark mode from 1.3.0, locale-selected headings, and a footer that is now on by default). No document, API, or configuration change is required, but the served HTML differs — redeploy intentionally.

### Added — `@takuhon/core` (schema 1.3.0 → 1.4.0, non-breaking)

- **`Project.role` (optional, localized).** New optional field on a project — the owner's role/relationship on it (e.g. "Author & maintainer"), a BCP-47 → string map like the `role` already on `Career` / `Membership` / `Volunteering`. When present, the standard renderer shows it as a short line under the project title.
- **`highlights[]` (optional, top-level content section) + `settings.highlightsIntro`.** A new top-level array for curated "picked posts" (the bio prototype's social highlights), each `{ id, platform, url, image, alt, title?, description?, postedAt?, tags?, order?, visibility? }` with `alt` a required localized map and `image` a `uri-reference` (so self-hosted, root-relative assets are allowed). `settings.highlightsIntro` is an optional localized lead line. `id`s are unique, `order` is canonically sorted, and item/section `visibility` participates in the privacy filter. Highlights are exposed over MCP; they are deliberately **excluded** from JSON-LD and the CV export.
- **`settings.sectionOrder` + `settings.sectionLabels`.** `sectionOrder` is an optional ordered list of section keys that reorders the rendered page (listed keys first in the given order, the rest trailing in the default order; unknown/duplicate keys are filtered). `sectionLabels` is an optional closed map that overrides any section or chrome heading per locale. A new `LABEL_KEYS` / `SectionLabelOverrides` taxonomy in core is the single source of truth for both.
- **`Volunteering.secondaryLink` (optional).** An optional `{ url, label? }` on a volunteering entry (the bio prototype's GitHub-organization pill, generalized) — the renderer draws it as a small pill with a brand glyph resolved from the URL host, with no vendor hard-coding.
- **`Skill.label` accepts a localized map.** `Skill.label` is now `anyOf` a plain string (unchanged) or a BCP-47 → string localized map, so a skill can carry per-locale labels. A legacy plain-string label round-trips untouched and upgrades to a single-locale map only on re-save. `resolveLocale` collapses the map to the resolved locale's string.
- **`settings.appearance.colors.heading` (optional).** A new overridable color token for the "About" section's Markdown sub-headings (`##`/`###`), in both `colors` and `colorsDark`. Reuses the existing `CssColor` allowlist. It defaults to a deep indigo (light) / soft indigo (dark) so sub-headings stand out; override it to re-tint them.

  Every one of the above is an optional/additive change, so a valid 1.3.0 document is a valid 1.4.0 document unchanged; the closed schema stays closed (`additionalProperties` remains `false`, no new `required`). `SCHEMA_VERSION` becomes `1.4.0`, `$id` advances to `/schemas/1.4.0/`, a `v1.3.0-to-v1.4.0` migration (a pure version stamp) is added, and the validator is regenerated.

### Added — `@takuhon/api`

- **Composition seam on `renderProfileHtml`.** Three additive, optional inputs let a host extend the standard renderer without forking it: (1) **`slots`** — first-party raw HTML injected verbatim at three fixed points (`head` after the `<style>` block, `mainEnd` after the last section, `bodyEnd` before `</body>`); slot content is intentionally not escaped because it is the host's own trusted markup, never profile/user data. (2) **`labels`** — a partial override map for section headings and chrome aria-labels, merged over the built-in defaults. (3) **`omitSections`** — suppress a section's visible rendering only; the embedded JSON-LD is still generated from the complete document. A caller that passes none of these gets byte-identical output apart from an always-on skip link. Also adds a visually-hidden `<a class="skip-link" href="#main">` targeting `<main id="main">`.
- **Markdown "About" section.** `renderProfileHtml` renders `profile.bio` as a dedicated "About" `<section>` using a minimal, dependency-free Markdown subset — `##`→h3, `###`→h4, `---`→hr, `- `→list, `**bold**`, and blank-line paragraphs. Every text node is HTML-escaped before the ASCII `**` markers are applied, and there is deliberately no inline-link/image syntax. A plain-prose bio degrades to a single paragraph. The heading is localizable via `labels.about` and suppressible via `omitSections`. `<meta name="description">` and the JSON-LD still read `profile.bio` from the document unchanged.
- **Highlights carousel.** When `highlights[]` is present the renderer emits a scroll-snap horizontal carousel (self-hosted images, a platform badge resolved from a known key / the URL host / raw text, a stretched link, a localized call to action, and the optional `settings.highlightsIntro` lead). The heading is overridable via `sectionLabels`.
- **Volunteering `secondaryLink` pill + built-in brand glyphs.** Volunteering entries render `secondaryLink` as a pill whose glyph is resolved from the URL host (with a `span` fallback for an unsafe URL and a host-derived label when `label` is absent).

### Changed — `@takuhon/api` (default layout is now the bio design — behavior change, no schema change)

- **The standard renderer's default layout is the bio design.** Links split into a prominent **Featured** grid at the top and an **other links** heading section pinned to the bottom; the default section order follows the bio prototype (about → experience → projects → volunteering → skills → activity → … → highlights → contact, with other-links last); volunteering renders as a single-line head with dates hidden; roles show as accent badges; and a footer is emitted by default (`showPoweredBy !== false`) with a license line. Section headings ship as built-in **Japanese/English packs selected automatically from the resolved locale** — an explicit reversal of 1.4.0's earlier "locale-agnostic, English-only default" seam decision (the `labels` override seam is preserved and still wins). Date-range separators are locale-dependent (`〜` for Japanese, an en-dash elsewhere). The About section's Markdown sub-headings pick up the built-in `--takuhon-color-heading` color, and experience-entry titles render as lightweight sub-headings (kept as `<h3>` for accessibility, styled down to body size). The small trailing link **type pill** (e.g. "GitHub") now appears only on a link with **no owner `label`** — whose visible text falls back to the URL and needs the platform named; an explicit label is authoritative and the brand glyph already signals the platform, so a labeled link (including a localized label such as "ブログ") renders clean without a redundant English pill. Every adapter that uses `renderProfileHtml` gains this default look; the turnkey output is no longer byte-identical to 1.3.0 (see the behavior-change note above).

### Added — `@takuhon/cloudflare` & `@takuhon/vercel`

- **Adapter `render` passthrough + host CSP extension.** `createTakuhonWorker` (Cloudflare) and `createTakuhonVercelApp` (Vercel) accept an optional `render?: PublicRenderOptions` — the renderer's `slots` / `labels` / `omitSections` plus an optional `csp` — threaded straight through to the server-rendered profile page. `render.csp` (`scriptSrc` / `connectSrc` / `workerSrc` / `scriptHashes`) widens **only** the profile page's Content-Security-Policy so host-injected slots (a PWA service-worker registration, an analytics beacon) can load their scripts; every other route keeps the strict `'self'` policy. Host tokens are validated (printable-ASCII source expressions, no directive/header injection, the blanket relaxations `'unsafe-inline'` / `'unsafe-eval'` rejected, `scriptHashes` limited to CSP hash expressions) and rejected tokens are dropped with a `console.warn`. Absent `render`, the page and its CSP are byte-identical to before.

### Lockstep version bump

- All eleven publishable artifacts bump from `1.3.0` to `1.4.0` — the nine scoped npm packages (`@takuhon/core`, `@takuhon/api`, `@takuhon/ui`, `@takuhon/activity`, `@takuhon/mcp`, `@takuhon/cli`, `@takuhon/contact`, `@takuhon/cloudflare`, `@takuhon/vercel`), the bare-name `takuhon` redirect, and the `create-takuhon` initializer. The scaffold's pinned `@takuhon/*` caret range (`TAKUHON_DEP_RANGE`) advances to `^1.4.0`. (`apps/admin`, `apps/playground`, `adapters/static`, and `adapters/wordpress` are private and not published.)
...
[Unreleased]: https://github.com/takuhon-dev/takuhon/compare/v1.4.0...HEAD
[1.4.0]: https://github.com/takuhon-dev/takuhon/compare/v1.3.0...v1.4.0
