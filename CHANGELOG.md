...
- **The standard renderer's default layout is the bio design.** Links split into a prominent **Featured** grid at the top and an **other links** heading section pinned to the bottom; the default section order follows the bio prototype (about → experience → projects → volunteering → skills → activity → … → highlights → contact, with other-links last); volunteering renders as a single-line head with dates hidden; roles show as accent badges; and a footer is emitted by default (`showPoweredBy !== false`) with a license line. Section headings ship as built-in **Japanese/English packs selected automatically from the resolved locale** — an explicit reversal of 1.4.0's earlier "locale-agnostic, English-only default" seam decision (the `labels` override seam is preserved and still wins). Date-range separators are locale-dependent (`〜` for Japanese, an en-dash elsewhere). Every adapter that uses `renderProfileHtml` gains this default look; the turnkey output is no longer byte-identical to 1.3.0 (see the behavior-change note above).
