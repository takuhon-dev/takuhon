...
Design-token re-skin seam, **a refreshed default look with built-in dark mode**, **brand-logo links with a featured/other split**, and **richer section layouts (an experience timeline and project cards)** for the standard renderer — the first upstream steps of the "bio design generalization" track (bring the bio prototype's design into the standard product so bio can shed its bespoke renderer and ride the turnkey path). Non-breaking: a valid 1.1.0 document is a valid 1.2.0 document unchanged.
