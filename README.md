...
ownport lets you own your profile — bio, SNS links, career history, projects, skills — as a portable JSON document, and publish it as a mobile-first profile page plus a public API (including JSON-LD for AI agents and search engines).

The canonical data lives in `ownport.json`. Storage backends (Cloudflare KV, static file, etc.) are pluggable via adapters.
...
- Profile content (`ownport.json`) license is chosen by the instance owner at scaffolding time (`meta.contentLicense`, no default).
